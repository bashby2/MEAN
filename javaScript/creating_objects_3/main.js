function VehicalConsturctor(name, wheels, passengers, speed){
  ...
  // PUBLIC
  const charNum='ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890'
  ...
  function vinMaker(){
    var vin='';
    for( var i=0; i < 17; i++ ){
    vin += charNum.charAt(Math.floor(Math.random() * charNum.length));
    }
  return vin;
  }
}

// moving all modes out of the constructor
VehicalConsturctor.prototype.makeNoise = function(){
  console.log('I am a ' + this.name + ', I have ' + this.wheels + ' wheels and can carry ' + this.passengers + ' passangers! ZOOOM');
}
VehicalConsturctor.prototype.move = function(){
  this.updateDistanceTravelled();
  this.makeNoise;
  console.log('The ' + this.name + ' is moving, with a speed of ' + this.speed)
}
VehicalConsturctor.prototype.checkMiles = function(){
  console.log(this.distance_travelled);
}
VehicalConsturctor.prototype.updateDistanceTravelled = function() {
